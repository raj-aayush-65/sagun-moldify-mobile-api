...
import { PASSWORD_SALT_LENGTH } from '../../common/utils/password.constants';
...
export class AuthService {
  ...
  async validateUser(email: string, password: string): Promise<User | null> {
    ...
    // The decrypted password is "password + salt" (salt is 16 chars as generated by the app)
    // Extract just the password for bcrypt comparison
    const actualPassword = decryptedPassword.slice(0, -PASSWORD_SALT_LENGTH);

    this.logger.debug(`Password length after removing salt: ${actualPassword.length}`);

    if (user && (await bcrypt.compare(actualPassword, user.passwordHash))) {
      return user;
    }
  ...
  async signup(signupDto: SignupDto, creatorId?: string) {
    ...
    // The decrypted password is "password + salt" (salt is 16 chars as generated by the app)
    // Extract just the password for hashing (remove the salt)
    const actualPassword = decryptedPassword.slice(0, -PASSWORD_SALT_LENGTH);
